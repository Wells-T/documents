...
>?CAM相关概念详见 [子账号获取访问授权](https://cloud.tencent.com/document/product/1364/56269)
...
>? QcloudAccessForTSERole：代表该策略仅供腾讯云微服务引擎（TSE）服务角色（TSE_QCSRole）进行关联，用于TSE临时访问云服务资源。
...
>?  QcloudAccessForApiGateWayRoleInCloudNativeAPIGateway：该策略仅供API网关（ApiGateWay）服务角色（ApiGateWay_QCSRole）进行关联，用于IoTHuB访问其他云服务资源。
...
5. 单击**确定**完成授权。该策略会显示在用户的策略列表中。
6. 重复2-5，完成 QcloudAPIGWReadOnlyAccess 策略的绑定。
>? QcloudAPIGWReadOnlyAccess：API网关（API Gateway）只读访问权限，包括云监控（MONITOR）部分权限。
